...
- **Cumulative forever.** Tracks total RX / TX / combined bytes since the moment of
  installation — multi-terabyte correct (int64 end-to-end, byte counts sent as strings so the
...
Requires Go 1.23+ and Node 18+ (Node 22 recommended).
...
| `GET /api/live/stream` | Server-Sent Events stream of live samples |
